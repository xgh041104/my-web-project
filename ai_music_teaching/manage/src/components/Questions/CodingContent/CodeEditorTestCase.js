...
const CodeEdtorTestCase = ({ language, codeTestOnChange, codeTestCaseList }) => {
    ...
    const columns = [
        {
            title: "序号", dataIndex: "key", key: "case",
            render: (_, record) => 'case' + record.case
        },
        { title: "测试代码", dataIndex: "codeTestContent", key: "codeTestContent" },
        { title: "预期输出", dataIndex: "output", key: "output" },
        {
            title: "操作",
            key: "action",
            render: (_, record) => (
        ...
        <div style={{ width: "40vw" }}>
            <Modal title={editingTest ? "编辑测试用例" : "新增测试用例"} open={modalVisible} style={{ minWidth: '50vw' }}
                ...
                <Form onFinish={handleSubmit} form={form}>
                    <Form.Item name="codeTestContent" label="输入测试代码" rules={[{ required: true, message: "请输入测试代码" }]}>
                        <AceEditor
                            mode={language}
                            theme="monokai"
                    ...
                    <Form.Item name="output" label="预期输出" rules={[{ required: true, message: "请输入预期输出" }]}>
...
export default CodeEdtorTestCase
