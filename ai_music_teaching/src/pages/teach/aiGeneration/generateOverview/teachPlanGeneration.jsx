...
export default function SyllabusGeneration() {
  ...
  const [text, setText] = useState('教学目标;教学重点;');
  ...
  const options = [
    ...
    { key: 'opt9', label: '教学反思' },
    {
      ...
      label: '教学过程设计',
      children: [
        { key: 'opt3-1', label: '讲授环节' },
        { key: 'opt3-2', label: '互动环节' },
        { key: 'opt3-3', label: '布置作业' },
      ],
    },
    {
      key: 'opt2',
      label: '学习活动安排',
      children: [
        { key: 'opt2-1', label: '个体学习任务' },
        { key: 'opt2-2', label: '合作学习任务' },
        { key: 'opt2-3', label: '展示与分享方式' },
      ],
    },
    {
      key: 'opt1',
      ...
      children: [
        {
          ...
          label: (
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              />
              <h3 style={{ margin: '0 8px 0 4px' }}>课时</h3>
              {/* 红字提示 */}
              {weekError && (
  ...
  const handleCheck = (checkedKeysValue, e) => {
    ...
    if (e.checked) {
      ...
      if (key === 'opt1-1') regex = new RegExp(`${weekContent}课时`, 'g');
  ...
  const handleAllToggle = () => {
    ...
    if (selectedKeys.size === allKeys.length) {
      ...
      const allText = allKeys.map(k => {
        if (k === 'opt1-1') return `${weekContent}课时`;
  ...
  const handleAllContent = () => {
    if (titleContent === '') {
      message.warning('请输入课程主题');
      return;
    }
    const totalText = `课程主题为:${titleContent}该教案的要求:${text}`;
    dispatch({
    ...
    <div className={styles.container}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={handleAllToggle} style={{ 
          ...
          <strong>{selectedKeys.size === getAllOptions().length ? '取消全选' : '全选'}</strong>
        </button>
      </div>
