...
export default function MusicTheory() {
  ...
  const [selectedNotes, setSelectedNotes] = useState({ startChar: 0, endChar: 0 });//选中的音符
  const [defaultAbcStr, setDefaultAbcStr] = useState(`X:1\nQ:100\nM:4/4\nL:1/64\nK:C\n%%MIDI program 0\n| | | | |`);
  ...
  useEffect(() => {
    if (abcjs.synth.supportsAudio()) {
      ...
      const visualObj = abcjs.renderAbc(container.current, defaultAbcStr, defaultOptions);
  ...
  function clickListener(abcelem, tuneNumber, classes, analysis, drag, mouseEvent) {
    ...
    setSelectedNotes({ startChar: abcelem.startChar, endChar: abcelem.endChar });
  ...
  const appendNotes = (section, note) => { //填充乐谱小节
  ...
  function initNote(timeSigns, clefBtns) {
    ...
    if (clefBtns === 'big') { //大谱表
      ...
      var abc = basicAbcStr + highPrefixStr + highSectionList.map((item) => item.notes).join('| ') + "|" + "\n" + lowPrefixStr + lowSectionList.map((item) => item.notes).join('| ') + "|";
      ...
      basicAbcStr = `X:1\nQ:100\nM:${timeSigns}\nL:${minNote}\nK:${keySign} ${clefBtns}\n%%MIDI program ${instrumentType}\n`;
      var abc = basicAbcStr + sectionList.map((item) => item.notes).join('| ') + "|";
      setDefaultAbcStr(abc);
    }
    tempBeats = 0; //临时节拍数
  ...
  const restoreNoteId = (pitchStr) => {
    ...
    //计算 noteId
    var pitchOffset = Object.keys(noteList).find(key => noteList[key] === pitchStr) - 1;
  ...
  //添加音符
  const addNote = (section, noteBtn, note, beatNum, beatValue, tempBeats, partIndex) => {
    // console.log(section)
    for (var i = 0; i < section.length + 1; i++) {
      // if (i >= 4) return tempBeats, message.warning("乐谱已满，无法继续添加小节");
      if (i >= 4 && i === section.length) {//超出加小节，4是初始乐谱小节数，
        section.push({ beats: 0, notes: " " });
      }

      if (i % 4 === 0 && i === section.length - 1 && section[i].beats === 0) {//每4小节换行
        section[i].notes += "\n";
      }
      ...
      if (section[i].beats + noteBtn <= beatNum * beatValue) {//容纳小节
        if (section[i].notes.length == 1) {//新小节将拍数置为0
          tempBeats = 0;
        }

        if (section[i].notes.lastIndexOf(" ") !== -1) {
          var noteList = section[i].notes.substring(section[i].notes.lastIndexOf(" "), section[i].notes.length);
          const numbers = noteList.match(/\d+/g);

          tempBeats = numbers ? numbers.reduce((acc, num) => acc + parseInt(num), 0) : 0;
  ...
  const addNoteAtEnd = (notePitch) => {
    ...
    if (tempNoteBtn > beatNum * beatValue) return message.warning("超出拍数，请重新选择音符");

    if (clefBtn === 'big') {
      if (notePitch.indexOf(",") !== -1) {//低音区
        lowTempBeats = addNote(lowSectionList, tempNoteBtn, notePitch, beatNum, beatValue, lowTempBeats, lowPartIndex);
      } else {//高音区
        highTempBeats = addNote(highSectionList, tempNoteBtn, notePitch, beatNum, beatValue, highTempBeats, highPartIndex);
      }
      setDefaultAbcStr(basicAbcStr + highPrefixStr + highSectionList.map((item) => item.notes).join('|') + "|" + "\n" + lowPrefixStr + lowSectionList.map((item) => item.notes).join('| ') + "|")
    } else {//单谱表
      tempBeats = addNote(sectionList, tempNoteBtn, notePitch, beatNum, beatValue, tempBeats, partIndex);
      setDefaultAbcStr(basicAbcStr + sectionList.map((item) => item.notes).join('|') + "|")
    }
  ...
  const checkChord = (lists) => {
    const list = lists.map((item) => { return item.noteId })
    var notePitch = "";
    ...
    if (n === 3) {
      ...
      if (chordName) {
        ...
        notePitch = "[" + calcNotePitch(list[0]) + calcNotePitch(list[1]) + calcNotePitch(list[2]) + "]";
        addNoteAtEnd(notePitch)
      } else {
        setNoteType("未知的和弦类型");
    ...
    if (n === 4) {
      ...
      if (chordName) {
        setNoteType(chordName);
        notePitch = "[" + calcNotePitch(list[0]) + calcNotePitch(list[1]) + calcNotePitch(list[2]) + calcNotePitch(list[3]) + "]";
        addNoteAtEnd(notePitch)
      } else {
        setNoteType("未知的和弦类型");
  ...
  const changeKeySignature = (keySignature) => {
    const index = keySignatures.findIndex(item => item.value === keySignature);
  ...
  const handDelete = () => {
    ...
    setDefaultAbcStr(basicAbcStr + section.substring(0, lastSecondIndex) + section.substring(lastIndex, section.length - 1));
  ...
  const deleteNote = (section, start, end, tempBeats) => {
    ...
    while (start >= 0) {
      ...
      if (start >= j) {
        ...
        //删除小节对应的节拍数
        beat = numbers ? numbers.reduce((acc, num) => acc + parseInt(num), 0) : 0;

        section[i].beats -= beat;

        //删除小节的单一节拍数

        var startIndex = section[i].notes.lastIndexOf(" ", start) !== -1 ? section[i].notes.lastIndexOf(" ", start) : 0;
        var endIndex = section[i].notes.indexOf(" ", end) !== -1 ? section[i].notes.indexOf(" ", end) : section[i].notes.length;
        ...
        section[i].notes = section[i].notes.substring(0, start) + " " + section[i].notes.substring(end, section[i].notes.length);
  ...
  const deleteSelectNote = () => {
    if (clefBtn === 'big') { //大谱表
      ...
      var highNotesLength = n + highSectionList.reduce((sum, item) => sum + item.notes.length + 1, 0);

      if (selectedNotes.startChar < highNotesLength) {//高音部
        ...
        var start = selectedNotes.startChar - highNotesLength - 1 - lowPrefixStr.length; //减去 "\n"和低音前缀
        var end = selectedNotes.endChar - highNotesLength - 1 - lowPrefixStr.length; //减去 "\n"和低音前拽
  ...
  const deleteSelectSection = () => {
    if (clefBtn === 'big') { //大谱表
      ...
      if (selectedNotes.startChar < highNotesLength) {//高音部
        ...
        var start = selectedNotes.startChar - highNotesLength - 1 - lowPrefixStr.length;
        var i = 0, sum = 0;
  ...
  return <div style={{
    width: 'auto', height: '90vh', left: '2vw', margin: '.3rem', display: 'flex',
    flexDirection: 'column', justifyContent: 'space-between'
  }}>
    {!unfold &&
      <div style={{ width: '100%', height: '35vh', overflow: 'auto', backgroundColor: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div style={{ width: '70%', marginLeft: '2vw', marginTop: '2vh' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{
              ...
              <div style={{
                width: '0.42rem', height: '0.76rem', display: 'flex', justifyContent: 'center', alignItems: 'flex-end',
                background: '#FC6874', boxShadow: '0px 0px 0.06rem 0px rgba(0,0,0,0.32)',
                borderRadius: '0.07rem',
              }}><p style={{
                writingMode: 'vertical-lr', fontFamily: 'SimHei',
                fontWeight: 400,
                fontSize: '0.2rem',
                color: '#FFFFFF',
              }}>音程</p></div>
                  ...
                  return (
                    <div key={"jinapu" + index} style={{
                      ...
                      {displayNotes(item).octaveNum > 5 &&
                        <div
                          style={{
                            writingMode: 'vertical-lr', width: '0.5rem', color: 'black',
                      ...
                      <div
                        style={{
                          color: 'black', textAlign: 'center', fontSize: '0.3rem', fontWeight: 400, width: '0.4rem',
                        }}>
          ...
          <FiveDegreeCirculation activeRotation={activeRotation} onData={handleDataFromChild} />
          <Tabs className='music-theory-tab' activeKey={secondActiveKey} onChange={(key) => { setSecondActiveKey(key) }} items={secondType} style={{ fontSize: '.2rem' }} />
          {secondActiveKey == 0 &&
            <Space size={'large'}>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      width: '0.62rem', height: '0.62rem', cursor: 'pointer',
                      boxShadow: '0px 0px 6px 1px rgba(0,0,0,0.32)',
                      borderRadius: '0.06rem',
                      display: 'flex', justifyContent: 'center', alignItems: 'center'
                    }}
                    onClick={() => { setNoteBtn(item.value) }}
                  >
                    <div style={{ filter: item.value === noteBtn && noteBtn !== 16 || noteBtn !== 16 && item.value === 16 ? 'invert(100%)' : '' }}>
                      <img src={baseUrl + item.label} style={{ maxHeight: '0.4rem', width: item.value === 64 ? '0.25rem' : 'auto' }} />
                    </div>
                  </div>
                );
              })
              ...
              <div
                style={{
                  ...
                  width: '.62rem', height: '.62rem', cursor: 'pointer',
                  boxShadow: '0px 0px 6px 1px rgba(0,0,0,0.32)',
                  borderRadius: '0.06rem',
                  display: 'flex', justifyContent: 'center', alignItems: 'center'
                }}
                onClick={() => { setRestBtn(!restBtn) }}>
                <div style={{ filter: restBtn ? 'invert(100%)' : '' }}>
                  <img src={baseUrl + '/image/musicNote/left.png'} style={{ height: '.4rem' }} />
                  <img src={baseUrl + '/image/musicNote/right.png'} style={{ height: '.4rem' }} />
                </div>
              </div>
              <div style={{
                ...
                borderRadius: '0.06rem',
                display: 'flex', justifyContent: 'center', alignItems: 'center'
              }} onClick={() => { setDotBtn(dotBtn === 0.5 ? 0 : 0.5) }}>
              ...
              {sharpBtnList.map((item) => {
                return (
                  <div
                    style={{
                      background: sharpBtn === item.value ? '#FA5B39' : '#fff',
                      width: '.62rem', height: '.62rem', cursor: 'pointer',
                      boxShadow: '0px 0px 6px 1px rgba(0,0,0,0.32)',
                      borderRadius: '0.06rem',
                      display: 'flex', justifyContent: 'center', alignItems: 'center'
                    }}
                    onClick={() => { setSharpBtn(sharpBtn === item.value ? "" : item.value) }}
                    key={'sharpBtn: ' + item.value}>
                    <div style={{ filter: sharpBtn === item.value ? 'invert(100%)' : '' }}>
                      <img src={baseUrl + item.label} style={{ maxHeight: item.value === '^^' ? '.23rem' : '0.3rem' }} />
                      {item.value === '__' && <img src={baseUrl + item.label} style={{ maxHeight: '0.3rem' }} />}
                    </div>
                  </div>
                )
              })}
              ...
              {chordList.map((item) => {
                return (
                  <Button
                    ...
                    type='button'
                    style={{
                      width: '1rem', height: '0.62rem', cursor: 'pointer',
                      boxShadow: '0px 0px 6px 1px rgba(0,0,0,0.32)',
                      borderRadius: '0.06rem',
                      display: 'flex', justifyContent: 'center', alignItems: 'center'
                    }}
    ...
    <div id="abcjs" style={{ height: unfold ? '66vh' : '27vh', overflowY: 'auto', backgroundColor: '#fff' }}>
      <div id='paper' ref={container} />
    </div>
    <div style={{ backgroundColor: '#fff' }}>
      <div style={{
        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        backgroundColor: '#1E1D22', padding: '0 0.1rem', height: '0.6rem'
      }}>
        <div id='audio' ref={audioRef}></div>
        <Select className='music-theory-select ' value={instrumentType} options={instrumentList} onChange={(e) => changeInstrument(e)}></Select>
        <div>
          <Select className='music-theory-select ' value={timeSign} options={timeSignatures} onChange={(e) => changeTimeSignature(e)}></Select>
          <Select className='music-theory-select ' value={clefBtn} options={clefList} onChange={(e) => changeClef(e)}></Select>
          <Select className='music-theory-select ' value={keySign} options={keySignatures} onChange={(e) => changeKeySignature(e)}></Select>
        </div>
        <div >
          <Button className='musicTheory-button' type='button' style={{}} onClick={handClear}>清空</Button>
          <Button className='musicTheory-button' type='button' onClick={() => { setUnfold(!unfold) }}>{unfold ? '收起' : '展开'}</Button>
          <Button className='musicTheory-button' type='button' onClick={() => { setIsDisPlayNote(!isDisPlayNote) }}>{isDisPlayNote ? '隐藏键盘提示' : '显示键盘提示'}</Button>
          <Button className='musicTheory-button' type='button' disabled={selectedNotes.endChar === 0} onClick={deleteSelectNote}>删除选中音符</Button>
          <Button className='musicTheory-button' type='button' disabled={selectedNotes.endChar === 0} onClick={deleteSelectSection}>删除选中小节</Button>
        </div>
      </div>
      <Piano
