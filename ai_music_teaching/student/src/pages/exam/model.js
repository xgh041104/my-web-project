...
const {
  GetStudentExamInfoAPI, GetStudentExamDetailsAPI, ExamStudentSumitAPI,
  GetSystemTimeAPI, GetStudentExamPaperOverAPI, UploadExamImage, GetExamNoticeByExamId
} = api;
...
export default {
  ...
  effects: {
    ...
    *queryExamDetail({ payload }, { call, put, select }) { //payload:{ExamId/ExamSessionId}
      const userInfo = yield select(_ => _.user.userInfo);
      if (!userInfo.isLogin) {
      ...
      const result = yield call(GetStudentExamDetailsAPI,
        { StudentId: userInfo.userId, ExamId: payload.ExamId, ExamSessionId: payload.ExamSessionId });
      if (result.code == 1) {
    ...
    *querySystemTime(_, { call, put }) {
      ...
      if (result.success) {
        ...
        if (Math.abs(offsetTime) > 60000) {// 大于一分钟就修正
