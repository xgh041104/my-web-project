...
class OpenSheetMusicDisplay {
  ...
  getInstrument() {
    ...
    let instruments = []
    for (var i = 0; i < this.osmd.GraphicSheet.ParentMusicSheet.instruments.length; i++) {
  ...
  setInstrument(index, midi_id, name, onLoadingInstrumentSuccessed, onLoadingInstrumentError) {
    for (var i = 0; i < this.osmd.GraphicSheet.ParentMusicSheet.instruments[index].Voices.length; i++) {
  ...
  clearAllNote() {
    ...
    for (var u = 0; u < measures.length; u++) {
      const measure = measures[u];

      for (var p = 0; p < measure.VerticalSourceStaffEntryContainers.length; p++) {
  ...
  updateNoteColor(note, color = '#36ba0d') {
    ...
    var hand = note.parentStaffEntry.parentStaff.idInMusicSheet + 1;
